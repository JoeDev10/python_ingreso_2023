...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        precio_producto = self.txt_importe.get()
        descuento = self.txt_descuento.get()
        
        precio_producto = float(precio_producto)
        descuento = float(descuento)
        
        precio_final_producto = precio_producto - (precio_producto * descuento / 100)

        alert("mensaje", f"el precio final del producto es: {precio_final_producto:.0f}")
